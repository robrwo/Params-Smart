...
sub _args { # based on self 0.15
    my $level = 2;
    my @c = ();
    if ( !defined($c[3]) || $c[3] eq '(eval)') {
        @c = do {
            @DB::args = ();
            caller($level);
	};
...
sub args {
  ...
  for (my $i=0; ($named && ($i < @args)); $i += 2) {
      if (!defined $args[$i]) { $named = 0 }
  }
  ...
  if ($named) {
    ...
    while ($self->{dynamic}) {
	  ...
	  if (exists $self->{names}->{$n}) {
            my $truename = $self->{names}->{$n}->{name};
	    $vals{$truename} = $args[$unknown{$n}+1];
	    if ($self->{names}->{$truename}->{callback}) {
	      $@ = undef;
	      eval {
		$vals{$truename} =
  ...
  unless ($named) {
	...
	$vals{$truename} = [ @args[$i..$#_] ];
	$i = $#_; # we don't want to use 'last'
      } else {
	$vals{$truename} = $args[$i];
      }
      if ($self->{names}->{$truename}->{callback}) {
	$@ = undef;
	eval {
  ...
  foreach my $name (keys %{ $self->{names} }) {
    ...
    croak "$name undefined" unless (%$info);
    unless (exists($vals{$name})) {
  ## TODO - warning if name undefined?
...
override memoization, use ParamsNC (non-caching) function:
